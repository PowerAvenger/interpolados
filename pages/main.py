...
df_spot_qh = download_esios_id(id,fecha_inicio,fecha_fin,agrupacion,agregacion)

#Resampleamos a spot h
df_spot_h = (
    df_spot_qh
        .set_index("datetime")
        .resample("H")
...
#Generamos df consumo coste h
df_comp_ch = pd.merge(
    df_ch,
    df_spot_h[["datetime", "spot"]],
    on="datetime",
    how="left"   # o "left" según lo que necesites
)
df_comp_ch["coste_real"] = df_comp_ch["consumo_real"] * df_comp_ch["spot"]/1000
print('df_comp_h')
print(df_comp_ch)
...
df_comp_cch = combinar_consumos_spot(df_comp_cch, df_spot_qh)
#Tabla DIARIA con los costes real e interpolado para ver la evolución mensual
df_comp_diario, mes_nombre = comparativa_mensual(df_comp_cch)
...
total_consumo_interpol = df_comp_cch['consumo_interpolado'].sum()
...
#METRIC SPOT
media_spot = round(df_spot_qh['spot'].mean(),2)
#METRIC COSTE
total_coste_real = round(df_comp_cch['coste_real'].sum(),2)
total_coste_interpol = round(df_comp_cch['coste_interpolado'].sum(),2)
...
with c2a:
    ...
    with st.container(height=altura_contenedor, border=True):
        st.subheader('4. Evolución diaria y mensual de la diferencia: Qh Real vs Interpolado', divider='rainbow')
        ...
        with c2a4:
            ...
            st.metric('Diferencia coste (€)', f'{diferencia_total_coste:,.2f}'.replace(",", "X").replace(".", ",").replace("X", "."), delta=f'{diferencia_total_coste_porc:,.2f}%'.replace(",", "."), delta_color='inverse')

    with st.container(height=altura_contenedor, border=True):
        st.subheader('5. Evolución diaria y mensual de la diferencia: Qh real vs H Real', divider='rainbow')
        c2a3,c2a4=st.columns([0.8,0.2]) 
        with c2a3:
            st.plotly_chart(graf_evol_coste_2, use_container_width=True)  
        with c2a4:
            st.metric("Coste QH (€)", f"{resumen['total_qh']:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
            st.metric("Coste H (€)", f"{resumen['total_h']:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
            st.metric("Diferencia (€)", f"{resumen['dif_total']:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."), delta=f"{resumen['impacto_pct']:.2f}%", delta_color='inverse')
            #st.metric("Impacto (%)", f"{resumen['impacto_pct']:.4f}%")
